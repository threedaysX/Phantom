...
public class WorkerClothes : InteractiveObjectModel
{
    private void Start()
    {
    ...
    void OnMouseUp()
    {
        if ((!DressingTable.Instance.combOK || !DressingTable.Instance.perfumeOK) && !EventSystem.current.IsPointerOverGameObject())
            if (!EventSystem.current.IsPointerOverGameObject())
            {
                ...
                WINDOWS.Instance.SetWindows("看起來是件工作服。\n是否要穿上?", "取消", "穿上");
            }
    }
    ...
    public override void GoOnBtnRight()
    {
        ...
        Dresses.Instance.ChangeCloth("WorkerClothes");
        //槌子掉出來
        ItemStatesManager.Instance.SetAvailable_CollectableItem(ItemNameDictionary.Hammer, AvailableState.Available);
    }
}
